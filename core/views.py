...
class QueryDayGraph(APIView):
    ...
    def get (self, request):
        ...
        for i in range(delta.days + 1):
            ...
            queryset = Expense.objects.filter(date__startswith = day)
...
class QueryMostRecentView(APIView):
    permission_class = [IsAuthenticated]
    def get(self, request):
        filtered = Expense.objects.all().filter(user=request.user).order_by('-id')[:5]
...
class QueryNetView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"expense": expense_sum , "income": income_sum, "net":net_value}, status=status.HTTP_200_OK)


class QueryCategoryView(APIView):
    permission_class = [IsAuthenticated]
    def get(self, request):
        categories = Category.objects.filter(date__month = str(current_month))
        data = []
        for i in categories:
